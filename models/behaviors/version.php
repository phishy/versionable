...
class VersionBehavior extends ModelBehavior {
	...
	function getVersionModel($model) {
		...
		/*
		if ($new->Behaviors->enabled('Version')) {
				...
				switch($association) {
					case 'hasAndBelongsToMany':
						if (!strstr($assoc['joinTable'], 'version')) {
							...
							#$model->{$association[$alias]['with']}->table = Inflector::singularize($assoc['joinTable']) . '_versions';
							$with = $model->{$association}[$alias]['with'];
							$model->{$with}->setSource(Inflector::singularize($assoc['joinTable']) . '_versions');
						}
						if (!$bypass) {
